...
start_marker = "// MARK: - Premium EI vector content"
end_marker = "// MARK: - Premium plant renderer"
start = text.index(start_marker)
end = text.index(end_marker)

replacement = r'''// MARK: - Canonical EI pet sprite content
...
enum HaloPetPose: String, CaseIterable, Identifiable {
...
contracts = Path("Halo/Core/ExtensionContracts.swift")
c = contracts.read_text()
old_picker = '''                        Picker("Pet style", selection: petStyleBinding) {
...
'''
new_picker = '''                        Label("Canonical pet artwork", systemImage: "photo.on.rectangle.angled")
...
debug_anchor = '''                        Toggle("Peek from screen edges", isOn: binding(\\.screenEdges)).disabled(!preferences.value.roam)
'''
debug_insert = debug_anchor + '''#if DEBUG
                        HaloPetDebugPanel()
#endif
'''
if debug_anchor in c and "HaloPetDebugPanel()" not in c:
    c = c.replace(debug_anchor, debug_insert, 1)

c = c.replace('''        case .petPeekUnder, .petPeekLeft, .petPeekRight: return .peek
''', '''        case .petPeekUnder: return .peek
        case .petPeekLeft: return .peekLeft
        case .petPeekRight: return .peekRight
''', 1)
c = c.replace('''        case .petObserve, .petRainWatch: return .observe
''', '''        case .petObserve: return .observe
        case .petRainWatch: return .umbrella
''', 1)
c = c.replace('''        case .petCurlUp, .petSleep, .petLaptopSleep: return .sleep
''', '''        case .petCurlUp, .petSleep: return .sleep
        case .petLaptopSleep: return .working
''', 1)
c = c.replace('''        case .petCoffee, .petYawn: return .tired
