...
enum HaloCIContextCatalog {
    static let fields: [HaloCIContextField] = [
        ...
        field("system.battery.onBattery", "boolean", "Whether the Mac is currently running from battery power.", since: "0.2"),
        field("system.power.source", "string", "Current power source: battery or ac.", since: "0.2"),
        ...
        field("context.event.kind", "string", "Most recent bounded context event, such as drag.entered, clipboard.changed, media.started, or power.chargingStarted.", since: "0.2"),
        field("context.event.source", "string", "Bounded source identifier for the most recent context event.", since: "0.2"),
        field("context.event.sequence", "number", "Monotonic in-process sequence number for context events.", unit: "count", since: "0.2"),
        field("context.event.ageSeconds", "number", "Age of the most recent transient context event.", unit: "seconds", since: "0.2"),
        field("input.drag.active", "boolean", "Whether a file/folder drag is currently targeting Halo.", since: "0.2"),
        field("input.drag.itemCount", "number", "Number of file-system items in the current Halo drag.", unit: "count", since: "0.2"),
        field("input.drag.fileCount", "number", "Number of files in the current drag after background classification.", unit: "count", since: "0.2"),
        field("input.drag.folderCount", "number", "Number of folders in the current drag after background classification.", unit: "count", since: "0.2"),
        field("input.drag.kind", "string", "Current drag kind: none, files, folders, or mixed.", since: "0.2"),
        field("input.drag.extensions", "string", "Comma-separated, bounded extension summary for the current drag; no paths are exposed.", since: "0.2"),
        field("input.drop.itemCount", "number", "Number of items in the most recent drop accepted by Halo.", unit: "count", since: "0.2"),
        field("input.drop.fileCount", "number", "Number of files in the most recent accepted drop.", unit: "count", since: "0.2"),
        field("input.drop.folderCount", "number", "Number of folders in the most recent accepted drop.", unit: "count", since: "0.2"),
