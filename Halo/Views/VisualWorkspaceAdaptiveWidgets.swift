...
private struct CaptureThumbnail: View {
    ...
    var body: some View { Group { if let image { Image(nsImage: image).resizable().scaledToFill() } else { ZStack { Color.primary.opacity(0.04); Image(systemName: "photo").foregroundStyle(.secondary) } } }.task(id: url) { image = await Task.detached(priority: .utility) { NSImage(contentsOf: url) }.value } }
}
