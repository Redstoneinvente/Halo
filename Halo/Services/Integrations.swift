...
final class ClipboardService: ObservableObject {
    ...
    func poll(enabled: Bool, excluded: String) {
        ...
        guard !sensitive.contains(where: { board.types?.contains(NSPasteboard.PasteboardType($0)) == true }) else { return }
        let text = board.string(forType: .string)
        HaloCIContextProviderEngine.shared.reportClipboardChanged(
            hasText: !(text?.isEmpty ?? true),
            textLength: text?.count ?? 0
        )
        guard let text, !text.isEmpty, text.utf8.count < 100_000 else { return }
    ...
    func copy(_ entry: ClipboardEntry) {
        ...
        HaloCIContextProviderEngine.shared.reportClipboardCopy(textLength: entry.text.count)
    }
}
...
final class MediaService: ObservableObject {
    ...
    /// Audio CI is visible only while playback is actually active. System/Safari already debounce
